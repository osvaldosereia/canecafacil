...
export function getNextBriefingQuestion(briefing: Briefing): string | null {
  const missing = getMissingBriefingInformation(briefing);

  if (missing.includes('creation_mode')) {
    return 'Você já tem uma imagem ou modelo como referência, ou quer criar a caneca do zero?';
  }

  if (missing.includes('reference')) {
    return 'Envie a imagem que você quer usar como referência para a caneca.';
  }

  if (missing.includes('creative_context')) {
    return 'Para quem ou para qual ocasião é a caneca? Pode me contar a ideia principal.';
  }

  if (missing.includes('style_or_creative_freedom')) {
    return 'Você prefere algum estilo específico ou posso criar com liberdade?';
  }
