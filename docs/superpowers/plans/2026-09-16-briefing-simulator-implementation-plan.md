...
**Files:**
- Create migration through Supabase named `secure_admin_simulator`; after Supabase returns its migration version, mirror the exact returned version in `supabase/migrations/<returned-version>_secure_admin_simulator.sql`.
...
- [ ] **Step 3: Write failing Admin-auth tests.** Cover missing bearer token, invalid token, authenticated non-admin, and authenticated admin. The test fake must prove membership lookup receives only the verified `user.id`, never a browser-supplied user ID.
...
- [ ] **Step 4: Prepare and apply the minimal Supabase authorization migration.** The migration content must enable RLS on the private table and expose only a service-role-readable lookup surface:
...
- [ ] **Step 7: Refactor `loadApiConfig`.** Keep `SUPABASE_URL` and `SUPABASE_SECRET_KEY` required; make WhatsApp fields optional; load `OPENAI_API_KEY`, `OPENAI_BRIEFING_MODEL`, and `ADMIN_ORIGIN` as optional capability settings. Reject partially configured WhatsApp only when code attempts to create the WhatsApp capability, not at server boot.

- [ ] **Step 8: Update `.env.example`.** Add `OPENAI_BRIEFING_MODEL=gpt-5.6-luna` and `ADMIN_ORIGIN=http://localhost:5173`; mark all `WHATSAPP_*` values as optional examples.
...
**Interfaces:**
- Produces: `BriefingExtraction`.
- Produces: `createEmptyBriefing()`.
- Produces: `mergeBriefing(previous, extraction)`.
- Produces: `getMissingBriefingInformation(briefing)`.
- Produces: `isBriefingReady(briefing)`.
- Produces: `getNextBriefingQuestion(briefing)`.
...
`set` adds newly learned facts; `replace` is reserved for explicit customer corrections. Array fields in `set` merge/deduplicate. Array fields in `replace` replace the targeted array. Existing scalar values are not silently overwritten by `set`.
...
- [ ] **Step 2: Write failing tests for deduplication and mandatory-text preservation.** Ensure `"Ana"` is never silently changed to `"ANA"` or paraphrased when it is mandatory text.

- [ ] **Step 3: Write failing readiness tests.** Encode this initial deterministic policy:
...
- [ ] **Step 2: Define a strict Structured Outputs schema.** The schema must include `set`, `replace`, `creativeFreedom`, `confidenceScore`, and `ambiguousFields`; no additional properties.
...
- [ ] **Step 4: Implement response validation.** Empty output, invalid JSON, unknown fields, invalid confidence range, or invalid enum values throw a normalized `BriefingProviderError` and do not return partial data.

- [ ] **Step 5: Run provider tests/typecheck/build.**

Run: `npm run test --workspace apps/api -- briefing-provider.test.ts && npm run typecheck --workspace apps/api && npm run build --workspace apps/api`
Expected: PASS.
...
- [ ] **Step 3: Register the route only with server-side dependencies.** Build the OpenAI provider from `OPENAI_API_KEY`/`OPENAI_BRIEFING_MODEL`. If OpenAI is not configured, route returns 503 without calling anything external.
